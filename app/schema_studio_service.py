...
def get_overview(db_key: str) -> dict[str, Any]:
    engine = get_engine(db_key)
    dialect = engine.dialect.name
...
def list_tables(db_key: str, search: str = '') -> list[dict[str, Any]]:
    engine = get_engine(db_key)
    dialect = engine.dialect.name
    inspector = sa_inspect(engine)
...
def get_table_rows(
    ...
    count_sql = sa.text(f'SELECT COUNT(*) FROM {qt}')
    data_sql = sa.text(f'SELECT * FROM {qt}{order_clause} LIMIT :limit OFFSET :offset')

    with engine.connect() as bind:
        total = bind.execute(count_sql).scalar() or 0
        result = bind.execute(data_sql, {'limit': per_page, 'offset': offset})
...
def get_relations(db_key: str) -> dict[str, Any]:
    engine = get_engine(db_key)
    inspector = sa_inspect(engine)
